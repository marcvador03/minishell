...
/*   Updated: 2024/12/18 00:34:52 by mfleury          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_pipe	*fill_pipes(t_pipe *p, char *line, int n, char **env)
{
	t_pipe	*tmp;
	int		i;
	char	*t_line;
	...
	while (i < n)
	{
		...
		if (p == NULL)
			tmp = p_lstnew(t_line, env);
		else
			tmp = p_lstadd_back(&p, t_line, env);
		if (tmp == NULL)
			return (NULL);
		p = tmp->head;
		i++;
...
int	subshell(t_shell *sh, char ***env)
{
	...
	p = fill_pipes(p, sh->s_line, sh->p_count, *env);
	if (p == NULL)
		return (-1);
	sh->pipes = p->head;
	if (sh->p_count == 1)
